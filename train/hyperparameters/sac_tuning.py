...
def buffersize_tune(env_id: str, log_dir: str, values: list, num_steps=300_000):
    device = "cuda" if t.cuda.is_available() else "cpu"
    eval_freq = 20_000
    ...
    policy_kwargs = dict(activation_fn=t.nn.Tanh, net_arch=dict(pi=[64, 64], qf=[64, 64]))
    log_dir = os.path.join(log_dir, "buffer_size")
    if not os.path.exists(log_dir):
        os.makedirs(log_dir, exist_ok=True)

    for buffer_size in values:
        model = SAC("MultiInputPolicy",
                    env=vec_env,
                    verbose=0,
                    tensorboard_log=log_dir,
                    ...
                    policy_kwargs=policy_kwargs,
                    device=device)
        model.learn(total_timesteps=num_steps, tb_log_name=f'buffer_size={buffer_size}', callback=eval_callback)
    print("Finished tuning buffer size.")
...
def learning_starts_tune(env_id: str, log_dir: str, values: list, num_steps=300_000):
    device = "cuda" if t.cuda.is_available() else "cpu"
    eval_freq = 20_000
    ...
    policy_kwargs = dict(activation_fn=t.nn.Tanh, net_arch=dict(pi=[64, 64], qf=[64, 64]))
    log_dir = os.path.join(log_dir, "learning_starts")
    if not os.path.exists(log_dir):
        os.makedirs(log_dir, exist_ok=True)
    ...
    for start in values:
        model = SAC("MultiInputPolicy",
                    ...
                    policy_kwargs=policy_kwargs,
                    device=device)
        model.learn(total_timesteps=num_steps, tb_log_name=f'learning_starts={start}', callback=eval_callback)
    print("Finished tuning learning starts.")


def entr_tune(env_id: str, log_dir: str, value_range: list, buckets: int, num_steps=300_000):
    device = "cuda" if t.cuda.is_available() else "cpu"
    eval_freq = 20_000
    ...
    ent_coeff_vals = list(np.linspace(value_range[0], value_range[1], buckets))  # Start with a list
    ent_coeff_vals.append("auto")

    for ent_coeff in ent_coeff_vals:
        if ent_coeff == "auto":
            tb_log_name = f'ent_coeff=auto'
        else:
            tb_log_name = f'ent_coeff={round(ent_coeff, 3)}'
